...
export default function ProductStockDetailPage({params} : {params : { id : string[]}}){
  const [warehouseProductItemEvents, setWarehouseProductItemEvents] = useState<WarehouseProductItemEvent[]>([]);
  ...
  <Stack gap={20}>
    ...
    <Chart
        chartType="LineChart"
        data={convertChartData('Time', 'Quantity', warehouseProductItemEvents)}
        width="100%"
        height="400px"
        legendToggle
      />
...
function WarehouseProductEventTable({ items: items }: { items?: WarehouseProductItemEvent[] }) {
  return (
    <TableContainer>
      <Table>
        <Thead>
        ...
        <Tbody>
          {items?.map((item) => (
            <WarehouseProductEventTableItem key={new Date(item.time).toLocaleTimeString()} item={item} />
          ))}
        </Tbody>
      </Table>
    </TableContainer>
...
function WarehouseProductEventTableItem({ item }: { item: WarehouseProductItemEvent }) {
  return (
    <Tr>
